...
function split(str, sep) {
  counter++;
  console.log(counter);
  var idx = str.indexOf(sep);  //idx = 4 for 'y'
  if (idx == -1) 
    return [str];
...
// console.log('SPLITTER', split('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'a'));
// 'happy birthday', 'y'
// ['happ', split(' birthday', 'y')]
...
// Base case
  ...
  // Recursive case
  return fibonacci(n - 1) + fibonacci(n - 2);  //runs twice for each 'n', so 2^n
}
...
function traverse(animalHierarchy, parent) {
  let node = {};
  animalHierarchy.filter(item => item.parent === parent) //[{id: 'Mammals', parent: 'Animals'}, {id: 'Reptiles', parent: 'Animals'}]
    .forEach(item => node[item.id] = traverse(animalHierarchy, item.id));  //node = {Animals: {Mammals: {}, Reptiles: {}}}
  return node;  
}
...
function traverseA(data, depth = 0) {
  ...
  Object.keys(data).forEach(key => {   //numkeys^nested levels
